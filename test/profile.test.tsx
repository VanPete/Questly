...
describe('Profile page', () => {
  ...
  it('shows sign-in prompt when unauthenticated and does not call fetch on save', async () => {
    global.fetch = vi.fn(() => Promise.resolve({ json: () => Promise.resolve({ profile: null }) })) as unknown as typeof fetch;
    render(<ProfilePage />);
  ...
  fireEvent.click(btn);
    await waitFor(() => expect((global.fetch as any).mock.calls.length).toBe(before));
  });
  ...
  it('loads display name and allows optimistic save (authenticated)', async () => {
    global.fetch = vi.fn((input: RequestInfo | URL, init?: RequestInit) => {
      ...
      return Promise.resolve({ json: async () => ({ profile: { display_name: 'Old' } }) } as any);
  ...
  fireEvent.click(saveBtn);
    ...
    await waitFor(() => {
      ...
      const postCall = calls.find((c: any[]) => c[1] && c[1].method === 'POST');
      expect(postCall).toBeTruthy();
      const [url, init] = postCall as [string, RequestInit];
      expect(url).toMatch(/\/api\/profile/);
      expect(init.method).toBe('POST');
      expect(JSON.parse(init.body as string)).toMatchObject({ display_name: 'New Name' });
    });
  });
});
