...
function Start({ returnTo }: { returnTo: string }) {
  ...
  const doOAuth = async (provider: "google" | "apple" | "discord") => {
    try {
      ...
      const scopes = provider === 'apple' ? 'name email' : 'openid email profile';
      ...
      <div className="w-full max-w-md">
        ...
        <form onSubmit={onContinue} className="space-y-3 rounded-2xl border p-4">
          ...
          <div className="space-y-2">
            <button title="Continue with Google" type="button" onClick={()=>doOAuth('google')} className="w-full px-4 py-2 rounded border hover:bg-neutral-50 text-left flex items-center gap-2">
              <span aria-hidden className="text-lg">🟢</span>
              <span>Continue with Google</span>
            </button>
            <button title="Continue with Apple" type="button" onClick={()=>doOAuth('apple')} className="w-full px-4 py-2 rounded border hover:bg-neutral-50 text-left flex items-center gap-2">
              <span aria-hidden className="text-lg">🍎</span>
